...
function getFullNames(runners) {
  let runnerNames = []
   runners.forEach(function(items) {
     return runnerNames.push(`${items.last_name}, ${items.first_name}`);
   })
   return runnerNames;
}
...
function firstNamesAllCaps(runners) {
  const firstCap = runners.map(function(items) {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const sizesArray = runners.filter(function(el) {
...
function tallyUpDonations(runners) {
  const sumDon = runners.reduce(function(acc, cv){
...
var  counterMakerWithLimit = function(maxV) {
  let count = 0
   function counter () {
    if (count <= maxV) {
      return count++
